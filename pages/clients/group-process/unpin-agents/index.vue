<template>
  <div>
    <div v-if="agents && employedAgents">
      <button
        @click="$router.go(-1)"
      ...
      <div class="w-full overflow-auto bg-white rounded-[12px] border mt-4">
        ...
        >
                      ...
                      <template #content>
                        <div class="overflow-auto font-normal">
                          <FilterItems
                            v-if="agents"
                            :data="agents"
                            :selectedItems="selectedAgentsToAllClients"
                            @onSelectItems="selectedAgentsToAllClients = $event"
                            :maxSelectionLimit="3"
                          />
                        </div>
                      </template>
          ...
          <tr
            ...
            v-for="(data, index) in employedAgents"
            :key="index"
          >
...
const selectedAgentsToAllClients = ref([]);
...
// methods
const onSave = async () => {
  for(let agent of employedAgents.value){
    agent.employees = agent.employees.filter(employee => !!employee.employee_id)
  }
  await clientsStore.saveAttachClientsAgents(employedAgents.value);
...
onMounted(async () => {
  ...
  [employedAgents.value, agents.value] = await Promise.all([
    clientsStore.getEmployedAgents(clientsIdsArr),
    clientsStore.getAgents(agentsParams.value),
  ]);
});

const getSelectedName = (agentId) => {
  if (agents.value) {
    const agent = agents.value.items?.find((agent) => agent.id === agentId);
...
watchEffect(() => {
  if (selectedAgentsToAllClients.value.length) {
    for (const employedAgent of employedAgents.value) {
      employedAgent.employees = selectedAgentsToAllClients.value.map(
        (agent) => ({
          employee_id: agent,
          visit: [],
        })
...
watchEffect(() => {
  if (selectedDaysToAllClients.value.length) {
    for (const employedAgent of employedAgents.value) {
      employedAgent.employees = employedAgent.employees.map((employee) => ({
        ...employee,

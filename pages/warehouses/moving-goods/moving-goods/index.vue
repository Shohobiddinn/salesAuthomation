<template>
  <div>
    ...
    <div class="flex justify-between mt-4">
      <page-title :title="'О перемещение'"></page-title>
    </div>
    <div class="flex gap-5">
      <div>
        <div class="fs-14">data</div>
        <d-input-date-picker class="w-50"/>
      </div>
      <div>
        <div class="fs-14">data</div>
        <input type="text" class="border rounded-lg h-10 w-50">
      </div>
    </div>
    <div>
      <DoubleTab first-tab-name="Dena" second-tab-name="Lorem ipsum" third-tab-name="Lorem ipsum" fourth-tab-name="Lorem ipsum" class="mt-6">
        <template #first>
          <WarehousesMovingGoodsDataTableGoods/>
        </template>
        <template #second>
...
<script setup>
import {ref} from "vue";
import GoodsDataTable from "~/components/warehouses/moving-goods/DataTableGoods.vue";
const filter = ref({

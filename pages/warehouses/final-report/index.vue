<template>
  <div>
    <div class="mt-2 text-[24px] font-[600] text-[#424F4F]">
      Склад финального отчета
    </div>
    <div class="mt-4">
      <div class="bg-white p-4 w-full rounded-[12px]">
        ...
        <div class="grid grid-cols-4 gap-4 mt-2">
          <multiply-selection
            :name="'Склад'"
            :filter.sync="filter.isCash"
            @toggle="(toggle) => (filter.isCash = toggle)"
          />
          <multiply-selection
            :name="'Категория'"
            :filter.sync="filter.isCash"
          ...
          <div class="flex justify-end w-full h-40px mt-8 ml-8">
            <button class="w-full py-2 rounded-lg bg-[#299B9B] text-white">
              Применить
            </button>
    ...
    <div class="flex justify-between mt-2">
      <page-title :title="'Продажа'" />
    </div>
    <div class="mt-4">
      <WarehousesFinalReportDetailDataTable />
    </div>
  </div>
</template>

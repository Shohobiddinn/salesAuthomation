...
export const useRejectsStore = (tableId: string) => defineStore(`rejects_${tableId}`, () => {
    ...
    const _loadData = async () => {
        const response = await $axios.post<AppResponse<RejectModel>>(APIRoutes.RejectsList,
            params,
            conf,
        );
    ...
    const add = async (value: RejectModel): Promise<RejectModel> => {
        if (value.id !== undefined) {
            const response = await $axios.put<RejectModel>(APIRoutes.RejectsWithId(value.id!.toString()),
                value,
                conf,
            );
            return response.data;
        }
        const response = await $axios.post<RejectModel>(APIRoutes.Rejects,
            value,
            conf,
        );
        return response.data;

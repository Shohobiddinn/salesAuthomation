  ...
  <div class="h-[500px] w-[490px] overflow-auto">
    ...
    <div v-for="config in configs" :key="config.Group" class="mb-10">
      ...
      <div v-for="item in config.Config" :key="item.Key">
        ...
        <div v-if="item.Value === 'time'" class="mt-2 flex flex-col gap-1">
          <TimePicker
            :selectedTime="inputValues[item.Key].Value"
            @onSelectedTime="inputValues[item.Key].Value = $event"
          />
        </div>
        ...
        >
          ...
          <menu-btn class="w-full">
            <template #btn>
              <m-btn-outlined
                class="border-grey flex items-center w-full gap-2 w-[400px]"
              >
                <fa-icon hash="&#xf078" />
                Выбрать
              </m-btn-outlined>
...
// methods
onMounted(async () => {
  if (agentsLimitStore.configByIdDialog) {
    agentConfigById.value = await agentsLimitStore.getConfigById(
      agentsLimitStore.configByIdDialog
    );
  }
  const [currenciesData, configsData] = await Promise.all([
    agentsLimitStore.getCurrencies(currenciesParams.value),
    agentsLimitStore.getAgentsConfigsAll(),
  ]);
    ...
    config.Config.forEach((item) => {
      if (
        ...
        values.value[item.Key] = {
          Value:
            agentConfigById.value?.length > 0
              ? getAgentValueById(item.Key)
              : "",
          Key: item.Key,
          Name: item.Name,
        };
        ...
        values.value[item.Key] = {
          Value:
            agentConfigById.value?.length > 0
              ? getAgentValueById(item.Key) === "true"
              : false,
          Key: item.Key,
          Name: item.Name,
        };
        ...
        values.value[item.Key] = {
          ...
          Value:
            agentConfigById.value?.length > 0
              ? getAgentValueById(item.Key).split(", ")
              : [],
          Key: item.Key,
        };
      }
...
const onSave = async () => {
  ...
  if (agentsLimitStore.configByIdDialog) {
    await postConfigToExactAgent(data);
    agentsLimitStore.closeConfigByIdDialog();
  } else {
    await postConfigToAllAgents(data);
    emit("closeDialog");
  }
};

const postConfigToAllAgents = async (postData) => {
  await agentsLimitStore.postAgentsConfigsAll(postData);
};

const postConfigToExactAgent = async (postData) => {
  await agentsLimitStore.postConfigById(
    agentsLimitStore.configByIdDialog,
    postData
  );
...
const getAgentValueById = (itemKey) => {
...
const onLoadElseCurrencyData = async () => {
  ...
  const data = await agentsLimitStore.getCurrencies(currenciesParams.value);

<template>
  <form class="grid grid-cols-1 w-full overflow-auto h-[450px] gap-2 pr-3" @submit.prevent="saveAll">
    <div class="flex-row">
      <label for="first_name" class="text-gray-3 fs-14 fw-4"> Имя <span class="text-red-3 fs-14">*</span></label>
    </div>
    <div class="flex-row">
      <d-input
        placeholder="Введите имя"
        type="text"
        id="first_name"
        class="w-full"
        :value="methodData.first_name"
        @change="(value) => (methodData.first_name = value)"
        required
      />
    </div>
    <div class="flex-row">
      <label for="last_name" class="text-gray-3 fs-14 fw-4"> Фамилия <span class="text-red-3 fs-14">*</span></label>
    </div>
    <div class="flex-row">
      <d-input
        placeholder="Введите фамилия"
        type="text"
        id="last_name"
        class="w-full"
        :value="methodData.last_name"
        @change="(value) => (methodData.last_name = value)"
        required
      />
    </div>
    <div class="flex-row">
      <label for="middle_name" class="text-gray-3 fs-14 fw-4"> Отчество </label>
    </div>
    <div class="flex-row">
      <d-input
        placeholder="Введите отчество"
        type="text"
        id="middle_name"
        class="w-full"
        :value="methodData.middle_name"
        @change="(value) => (methodData.middle_name = value)"
      />
    </div>
    <div class="flex-row">
      <label for="phone" class="text-gray-3 fs-14 fw-4"> Номер телефона <span class="text-red-3 fs-14">*</span></label>
    </div>
    <div class="flex-row">
      <d-input
        placeholder="Введите"
        type="tel"
        id="phone"
        class="w-full"
        :value="methodData.phone"
        @change="(value) => (methodData.phone = value)"
        required
      />
    </div>
    <div class="flex border-t border-[#E1E4E4] mt-4 justify-between">
      <div class="fs-14 mt-4">Выбрать цвет для KPI</div>
      <div class="flex mt-4 ml-4">
        <div class="input-color-container cursor-pointer">
          <input v-model="methodData.color" class="input-color" type="color" />
        </div>
      </div>
    </div>
    <div>
      <flex-col class="gap-1">
        <div class="flex-row">
          <div class="text-gray-3 fs-14 fw-4">Город</div>
        </div>
        <menu-btn class="w-full">
          <template #btn>
            <m-btn-outlined class="border-grey flex items-center w-full gap-2 flex justify-between">
              <div class="flex gap-2">
                <fa-icon hash="&#xf078" />
                Выбрать
              </div>
              <div>
                {{methodData.territory_ids.length}} из {{territories?.total_count}}
              </div>
            </m-btn-outlined>
          </template>
          <template #content>
            <div class="h-44 overflow-auto">
              <FilterItems
                :data="territories"
                :selectedItems="methodData.territory_ids"
                @onSelectItems="methodData.territory_ids = $event"
                @onLoadElse="onLoadElseTerritories"
              />
            </div>
          </template>
        </menu-btn>
      </flex-col>
    </div>
    <div class="flex-row">
      <span class="text-gray-3 fs-14 fw-4"> Склад для возврата <span class="text-red-3 fs-14">*</span></span>
    </div>
    <div class="flex-row">
      <select
        class="border w-full h-[42px] bg-[#FAFDFD] rounded-lg mt-[1px] mr-[18px] pl-[15px] outline-none"
        :class="invalidWarehouseId ? 'border-red-3' : 'border-grey'"
        v-model="methodData.warehouse_id"
      >
        <option value="">Выбрать</option>
        <option
          v-for="warehouse in warehouses"
          :key="warehouse.id"
          :value="warehouse.id"
        >
          {{ warehouse.name }}
        </option>
      </select>
      <small v-if="invalidWarehouseId" class="text-red-3">Склад для возврата обязательное поле</small>
    </div>
    <div class="flex justify-between items-center mt-1">
      <span class="text-gray-3 fs-14 fw-4"> Aктивный  </span>
      <label class="switch">
        <input
          v-model="methodData.is_active"
          type="checkbox"
        />
        <span class="slider round"></span>
      </label>
    </div>
    <div class="flex-row">
      <label for="code" class="text-gray-3 fs-14 fw-4"> Код </label>
    </div>
    <div class="flex-row">
      <d-input
        placeholder="Введите код"
        type="text"
        id="code"
        class="w-full"
        :value="methodData.code"
        @change="(value) => (methodData.code = value)"
      />
    </div>
    <div class="flex-row">
      <label for="login" class="text-gray-3 fs-14 fw-4"> Логин <span class="text-red-3 fs-14">*</span></label>
    </div>
    <div class="flex-row">
      <d-input
        placeholder="Введите логин"
        type="text"
        id="login"
        class="w-full"
        :value="methodData.login"
        @change="(value) => (methodData.login = value)"
        required
      />
    </div>
    <div class="flex-row">
      <label for="password" class="text-gray-3 fs-14 fw-4"> Пароль <span v-if="!expeditorsStore.editDialog" class="text-red-3 fs-14">*</span></label>
    </div>
    <div class="flex-row mb-14">
      <d-input
        placeholder="**********"
        type="password"
        id="password"
        class="w-full"
        :value="methodData.password"
        @change="(value) => (methodData.password = value)"
        :required="!expeditorsStore.editDialog"
      />
    </div>
    <div
      class="left-[-16px] custom-shadow w-[450px] flex pb-8 rounded-b-lg mt-2 px-10 pt-6 absolute bottom-[-42px] bg-white z-50 items-center"
    >
      <m-btn class="w-full" type="submit">
        {{ expeditorsStore.editDialog ? "Сохранять" : "Добавлять" }}
      </m-btn>
    </div>
  </form>
</template>

<script setup>
import { useNotification } from "@kyvg/vue3-notification";
const emit = defineEmits("closeDialog");

// store
const expeditorsStore = useExpeditorsStore("main");

// State
let methodData = ref({
  first_name: "",
  middle_name: "",
  last_name: "",
  phone: "",
  login: "",
  code: "",
  color: "#A16543",
  password: "",
  apk_version: "",
  application_access: false,
  is_active: true,
  territory_ids: [],
  warehouse_id: "",
});

const warehouses = ref();
const territories = ref();

const territoriesParams = ref({
  page: 1,
  page_size: 10,
  search: null,
  order_by: {
    field: "id",
    is_asc: true,
  },
});

const warehousesParams = ref({
  page: 1,
  page_size: 10,
  search: null,
  order_by: {
    field: "id",
    is_asc: true,
  },
});

const invalidWarehouseId = ref(null)

// Methods

onMounted(async () => {
  if (expeditorsStore.editDialog) {
    methodData.value = await expeditorsStore.getExpeditorDetail(
      expeditorsStore.editDialog
    ); // editDialog = tableId
  }
  const [territoriesData, warehousesData] = await Promise.all([
    expeditorsStore.getTerritories(territoriesParams.value),
    expeditorsStore.getWarehousesList(warehousesParams.value),
  ]);
  territories.value = territoriesData;
  warehouses.value = warehousesData?.items;
});

const saveAll = async (e) => {
  const { notify } = useNotification();
  if(!methodData.value.warehouse_id) {
    invalidWarehouseId.value = true
    return
  }
  try {
    await expeditorsStore.add(methodData.value);
    await expeditorsStore.refresh()
    notify({
      title: `Экспедитор успешно ${
        expeditorsStore.editDialog ? "изменено" : "добавлено"
      }`,
    });
    emit("closeDialog");
  } catch (e) {
    console.log(e);
    const errorMessage = e.response?.data?.Messages[0];
    if (
      errorMessage === `Login: ${methodData.value.login} is already exists.`
    ) {
      notify({
        title: `Логин ${methodData.value.login} уже существует!`,
        type: "error",
      });
    } else {
      notify({ title: "Ошибка при добавлении экспедитора!", type: "error" });
    }
  }
};

const onLoadElseTerritories = async () => {
  territoriesParams.value.page_size += 10;
  const data = await expeditorsStore.getTerritories(territoriesParams.value);
  territories.value = data;
};

watchEffect(() => {
  if(methodData.value.warehouse_id){
    invalidWarehouseId.value = false
  }
})

</script>

<style scoped>
.input-color-container {
  position: relative;
  overflow: hidden;
  width: 28px;
  height: 28px;
  border-radius: 8px;
}

.input-color {
  position: absolute;
  right: -8px;
  top: -8px;
  width: 56px;
  height: 56px;
  border: none;
}
::-webkit-scrollbar {
  width: 12px;
}

::-webkit-scrollbar-track {
  -webkit-box-shadow: inset 0 0 6px rgba(0, 0, 0, 0.3);
  border-radius: 10px;
}

::-webkit-scrollbar-thumb {
  border-radius: 10px;
  -webkit-box-shadow: inset 0 0 6px rgba(0, 0, 0, 0.5);
}
</style>

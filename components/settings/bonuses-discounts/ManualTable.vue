  ...
  <div class="rounded-lg bg-white border-grey py-3">
    ...
    <div class="mt-3 w-full overflow-auto">
      <data-table
        ...
        @sort="changeSortData"
        :sorted="bonusStore.paramsManual?.order_by"
      >
        <template #body>
          <c-tr v-for="(data, index) in bonusStore.dataManual?.items" :key="data">
            <c-td v-for="key in bonusStore.autoTemplate" :key="key">
              ...
              <div v-if="key.checked === true && key.key === 'valid_to'">
                {{ moment(data[key.key]).format("DD0-MM-YYYY") }}
              </div>
            </c-td>
    ...
    <div class="flex justify-between w-full">
      <div class="flex p-3 gap-2 items-center">
        ...
        <page-size-btn
          :current-size="bonusStore.paramsManual.page_size"
          @setPageSize="onChangePageSize"
        />
      </div>
      <div class="p-3">
        <page-index
          :available-pages="bonusStore.dataManual?.total_pages"
          :current-page="bonusStore.dataManual?.page_number"
...
const changeSortData = (val) => {
  bonusStore.sortData(val, "manaul");
};

const searchData = (val) => {
  bonusStore.search(val, "manual");
};
const onChangePageSize = (val) => {
  bonusStore.setPageSize(val, "manual");
};
const onChangeSetPage = (val) => {
  bonusStore.setPage(val, "manual");
};

// Hooks
onMounted(() => {
  bonusStore.getDataManual(isActive.value);
});
</script>

<template>
  <div class="rounded-lg bg-white border-grey py-3">
    <div class="flex flex-row gap-4 px-4 items-center">
      ...
      <table-filter :templates="discountStore.autoTemplate"></table-filter>
      <div>
        <search-input @change="discountStore.search" class="w-full h-38px" />
      </div>
      <div>
    ...
    <div class="mt-3 w-full overflow-auto">
      <data-table
        ...
        :headers="discountStore.autoTemplate"
        @sort="discountStore.sortData"
        :sorted="discountStore.params.order_by"
      >
        <template #body>
          <c-tr v-for="(data, index) in discountStore.data?.items" :key="data">
            <c-td v-for="key in discountStore.autoTemplate" :key="key">
            ...
            <c-td :key="index" class="">
              <div :key="index" @click="openDropdown(index)" class="relative drop">
    ...
    <div class="flex justify-between w-full">
      <div class="flex p-3 gap-2 items-center">
        ...
        <page-size-btn
          :current-size="discountStore.params.page_size"
          @setPageSize="discountStore.setPageSize"
        />
      </div>
      <div class="p-3">
        <page-index
          :available-pages="discountStore.data?.total_pages"
          :current-page="discountStore.data?.page_number"
          @setPage="discountStore.setPage"
        />
      </div>
    </div>
  </div>
  <transition name="modal">
    <div v-if="draggable">
      <d-modal @closeDialog="draggableDialog" :name="'Управление меню таблицы'">
        <drag-and-drop
          @change="change"
          :templates="discountStore.autoTemplate"
        ></drag-and-drop>
      </d-modal>
    </div>
  </transition>
...
// Store
const discountStore = useDiscountsStore(isActive.value.toString());
...
function change(param) {
  discountStore.autoTemplate = param;
...
function edit(id) {
  router.push({
    path: "/settings/bonuses-discounts/create-auto-discount",
    query: { id: id },
  });
}

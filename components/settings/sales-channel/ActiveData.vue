<template>
  <div class="rounded-lg bg-white border-grey py-3">
    <div class="flex flex-row gap-4 px-4 items-center">
      ...
      <table-filter :templates="salesChannelsStore.templates"></table-filter>
      <div>
        <search-input
          @change="salesChannelsStore.search"
          class="w-full h-38px"
        />
      </div>
      <div>
      ...
      >
        <template #body>
          <c-tr v-for="data in salesChannelsStore.data?.items" :key="data">
            <c-td v-for="key in salesChannelsStore.templates" :key="key">
...
function change(param: any) {
  props.salesChannelsStore.templates = param;

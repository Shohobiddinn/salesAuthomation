  ...
  <div>
    <div class="flex justify-between mt-4">
      <div class="flex gap-4">
        ...
        <div class="-mt-1.5">
          <page-title :title="'По категории продуктор: по SVR'" />
        </div>
      </div>
      <div class="flex flex-row gap-2">
        <button
          ...
          :class="
            sel.isActive && sel.index === index
              ? 'fs-14 fw-6 text-primary border-color-primary border-b-2'
              : 'fs-14'
          "
          @click="checkSelsct(index)"
        >
          {{ item.name }}
        </button>
      </div>
          ...
          <template #first>
            <div class="overflow-auto table-containers">
              <data-table :headers="headers" @sort="sortData" :sorted="sortedData">
                <template #body>
                  <template v-for="(data, index) in salesSupervisorCategoryReport?.items" :key="index">
...
const props =  defineProps({
  salesSupervisorCategoryReport:Object,
  headers:Array
})
...
function checkSelsct(index) {
  sel.value.isActive = true;
  sel.value.index = index;
}

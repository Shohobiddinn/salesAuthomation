<template>
  <div class="w-full overflow-auto pb-3 bg-white mb-2 rounded-lg border-grey">
    <table class="w-full rounded-t-large overflow-hidden whitespace-nowrap">
      <thead>
        <tr class="bg-lotion ">
          <template v-for="header in expandatureStore.templatesExpense" :key="header">
      ...
      <tbody>
        <template v-for="(data, index) in expandatureStore.data" :key="data">
          <tr class="border-t-1">
            <td v-for="key in expandatureStore.templatesExpense" :key="key" class="pl-3 tdc">
    ...
    <transition name="modal">
      <div v-if="editOPenmodal">
        <d-modal @closeDialog="editOPenmodal = false" :dataContainerWidth="'400px'" name="Ред. cтатья расхода">
          <DashboardExpenditureDialog :value="selectedDate" @closeModal="editOPenmodal = false" />
        </d-modal>
      </div>
    </transition>
  </div>
...
// Hooks
onMounted(async () => {
  await expandatureStore.getDataExpense()
})
</script>

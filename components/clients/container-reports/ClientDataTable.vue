<template>
  <div class="rounded-lg bg-white border-grey px-[2px]">
    <div class="flex mb-4 ml-4 flex-row gap-4 mt-4 items-center">
      ...
      <table-filter :templates="clientsTaraDebts?.templates"></table-filter>
      <div>
        <search-input @change="clientsTaraDebts.search" class="w-full h-38px" />
      </div>
      <div>
...
<script setup>
const props = defineProps({ clientsTaraDebts: Object });
// State
import { ref } from "vue";
...
function change(param) {
  props.clientsTaraDebts.templates = param;
  draggable.value = false;
}
